...
function ametex_get_elementor_schemes_data() {
...
function ametex_get_elementor_schemes_data_colors() {
	$schemes = ametex_get_elementor_schemes_data();
	$color   = $schemes['color'];
	$result  = [];
	foreach ( $color['items'] as $index => $item ) {
		$slug            = implode( '-', [ 'apack-color', str_replace( ' ', '-', strtolower( $item['title'] ) ) ] );
...
function ametex_get_elementor_schemes_data_fonts() {
	$schemes = ametex_get_elementor_schemes_data();
	$font   = $schemes['typography'];
	$result  = [];
	foreach ( $font['items'] as $index => $item ) {
		$slug            = implode( '-', [ 'apack-font', str_replace( ' ', '-', strtolower( $item['title'] ) ) ] );
		$result[ $slug ] = [ "name" => "--{$slug}", "value" => $item['value'] ];
	}
