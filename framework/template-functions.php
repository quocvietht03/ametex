		...
		if (is_category()) {
			...
			if (get_post_type() != 'post') {
				if (get_post_type() == 'fw-portfolio') {
					...
					$slug = (isset($post_type->rewrite) && is_array($post_type->rewrite) && isset($post_type->rewrite['slug'])) ? $post_type->rewrite['slug'] : get_post_type();
					echo '<a href="' . esc_url(home_url('/')) . '/' . $slug . '/">' . $post_type->labels->singular_name . '</a>';
					echo ' ' . $delimiter . ' ' . '<span class="current">' . get_the_title() . '</span>';
				}
			} else {
