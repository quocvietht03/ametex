 ...
 */
define("ITEM_ID","23986132");
if (!class_exists('BearsthemesCommunicator')):
  class BearsthemesCommunicator{
    var $baseUrl;
    var $message;
    function __construct($baseUrl='https://beplusthemes.com/help') {
    ...
    function registerDomain($purchaseCode, $domain) {
        $response = wp_remote_post( $this->baseUrl.'/api/v1/register-domain-by-purchase-code', array(
        	'method' => 'POST',
          'timeout' => 50,
        ...
        if ( is_wp_error( $response ) ) {
           ...
           $this->message = __("Something went wrong on unregister your domain: ", "ametex").$error_message;
           return null;
        } else {
           return true;
        }
    ...
    function unRegisterDomains($purchaseCode) {
      $response = wp_remote_post( $this->baseUrl.'/api/v1/unregister-domain-by-purchase-code', array(
        'method' => 'POST',
        'timeout' => 50,
    ...
    function getConnectedDomains($purchaseCode) {
      ...
      if ( is_wp_error( $response ) ) {
         $error_message = $response->get_error_message();
         $this->message = __("Something went wrong on get connected domain with your purchase code: ", "ametex").$error_message;
         return null;
      } else {
    ...
    function getPurchaseInformation($purchaseCode) {
      ...
      if ( is_wp_error( $response ) ) {
         $error_message = $response->get_error_message();
         $this->message = __("Something went wrong on get purchase information: ", "ametex").$error_message;
         return null;
      } else {
         $result = json_decode($response['body']);
         return $result;
    ...
    public function verifytheme_sanitize( $input )
    {
        ...
        if ($ok_purchase_code) {
          ...
          if($communicator->message){
            ...
            $message .= __("Invalid purchase code<br />","ametex");
            $message .= __("Invalid purchase code<br />","ametex");
          }
        }
