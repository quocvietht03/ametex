...
<article <?php post_class('bt-post__item'); ?>>
	...
	<div class="bt-post__content">
		<?php
			...
			if( !empty( $settings['show_social'] ) && !empty($social) ) {
				echo '<ul class="bt-post__socials">'.implode(' ', $social).'</ul>';
			}
			...
			if( !empty( $settings['show_read_more'] ) || ( !empty( $settings['read_more_text'] ) && !empty( $settings['read_more_icon'] ) ) ) { 
				...
				echo '<a class="bt-post__readmore" href="#">'.$settings['read_more_text'].$read_more_icon_html.'</a>';
			}
		?>
	</div>
</article>
