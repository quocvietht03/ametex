...
if( ! function_exists( 'ametex_import_pack_import_action_ajax_callback' ) ) {
    ...
    function ametex_import_pack_import_action_ajax_callback() {
        ...
        if( ! isset( $data['form_data'] ) || ! isset( $data['form_data'][$data['action_type']] ) || ! function_exists( $data['form_data'][$data['action_type']] ) || $data['form_data'][$data['action_type']] !== 'ametex_import_pack_backup_site_skip_func' ) {
            wp_send_json( [
                'success' => true,
                'type' => 'error',
                'message' => __( 'Demo import failed. Please open a ticket for support!', 'ametex' ),
            ] );
        } else {
